...
#Create a dictonary for each electrode with the electrode as the key and the ISI of all spike times as the values            
    ...
    worksheet.write("F1", "Burst Electrode")
    worksheet.write("G1", "Number of spikes")
    worksheet.write("H1","First spike time (sec)")
    worksheet.write("I1","Last spike time (sec)")
    worksheet.write("J1","Burst duration")
    ...
    for (k,v), (k2,v2) in zip(data_list.items(),ISIth_dict.items()):
        ...
        for item in range(len(burst_electrode)):
            worksheet.write(item+1,5,burst_electrode[item])
        
        for item in range(len(burst_number)):
            worksheet.write(item+1,6,burst_number[item])
        
        for item in range(len(burst_start)):
            worksheet.write(item+1,7,burst_start[item])
        
        for item in range(len(burst_end)):
            worksheet.write(item+1,8,burst_end[item])
        
        for item in range(len(burst_duration)):
            worksheet.write(item+1,9,burst_duration[item])
    ...
    worksheet.write("L1", "Number of electrode bursts")
    worksheet.write("M1", "Number of electrodes")
    worksheet.write("N1", "Beginning of network burst")
    worksheet.write("O1","End of network burst")
    worksheet.write("P1","Network burst duration")
    worksheet.write("Q1","Average electrode burst duration")
    worksheet.write("R1", "Number of spikes")
    ...
    for item in range(len(netburst_number)):
            worksheet.write(item+1,11,netburst_number[item])        
         
    for item in range(len(num_electrode)):
            worksheet.write(item+1,12,num_electrode[item])  
    
    for item in range(len(netburst_start)):
            worksheet.write(item+1,13,netburst_start[item]) 
    
    for item in range(len(netburst_end)):
            worksheet.write(item+1,14,netburst_end[item]) 
    
    for item in range(len(netburst_dur)):
            worksheet.write(item+1,15,netburst_dur[item]) 
    
    for item in range(len(elecburst_dur)):
            worksheet.write(item+1,16,elecburst_dur[item]) 
    
    for item in range(len(spike_number)):
            worksheet.write(item+1,17,spike_number[item]) 
